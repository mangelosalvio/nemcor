...
const validateInput = require("./../../validators/supplier-withdrawals");
...
const Model = SupplierWithdrawal;
const seq_key = "ww_no";
...
router.post("/paginate", (req, res) => {
  ...
  const form_data = {
    ...
    ...(!isEmpty(advance_search.po_no) &&
      validator.isNumeric(advance_search.po_no) && {
        "purchase_order.po_no": parseInt(advance_search.po_no),
      }),
      ...
      record
        ...
        .then(async (record) => {
          ...
          if (old_record.purchase_order?._id) {
            ...
            await update_inventory.updatePoStatus({
              _id: old_record.purchase_order?._id,
            });
          }
