...
const validateInput = require("./../../validators/sales-orders-cement");
...
const {
  ...
  STATUS_PARTIAL,
  DELIVERY_TYPE_OWN_DELIVERY,
} = require("../../config/constants");
const {
  createDeliveryReceiptFromSalesOrder,
  createDeliveryReceiptFromSalesOrderCement,
...
const Model = SalesOrderCement;
const seq_key_counter = "so_cement_no";
const seq_key = "so_no";
...
router.put("/", (req, res) => {
  ...
  if (user?.department?._id) {
    counter_promise = CompanyCounter.increment(
      seq_key_counter,
      user?.department?._id
    );
  } else {
    counter_promise = Counter.increment(seq_key_counter);
  }
...
router.post("/for-bundling", (req, res) => {
  const { period_covered, department } = req.body;
  ...
  Model.aggregate([
    {
      $match: {
        ...
        delivery_type: DELIVERY_TYPE_OWN_DELIVERY,
        ...(department?._id && {
    ...
    },
    {
      $project: {
        ...
        so_delivery_area: "$delivery_area",
        company: "$company",
        customer: 1,
        stock: "$items.stock",
        ...
        is_open_quantity: "$items.is_open_quantity",
      },
    },
  ])
  ...
  Model.aggregate([
    {
      $match: {
        ...
        ...(req.body.stock?._id && {
          "items.stock._id": ObjectId(req.body.stock._id),
        }),
      },
    },
    {
...
router.delete("/:id", (req, res) => {
  Model.findByIdAndUpdate(
    ...
    .then((record) => {
      //delete delivery receipt attached to sales order
      DeliveryReceipt.updateOne(
        {
          "sales_order._id": ObjectId(record._id),
        },
        {
          $set: {
            deleted: {
