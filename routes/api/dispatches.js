...
const validateInput = require("./../../validators/dispatches");
...
router.get("/:id/print-bundles", async (req, res) => {
  ...
  await printing_functions.printBundles({ _id: req.params.id, bundle });
...
router.post("/:id/update-bundle-status", (req, res) => {
  const filtered_body = filterId(req);
  const user = req.body.status.user;
...
router.post("/load-bundle", (req, res) => {
  ...
  if (bundle_arr?.[0] && bundle_arr?.[1]) {
    ...
    Dispatch.aggregate([
      ...
      .then((records) => {
        if (records.length <= 0) {
          return res.status(401).json({ msg: "No Bundle Code Found" });
        }
...
router.post("/paginate", (req, res) => {
  ...
  const form_data = {
    ...
    ...(advance_search.location && {
      "items.customer.location._id": ObjectId(advance_search.location._id),
    }),
