...
const title = "SRV Form";
const url = "/api/stocks-receiving/";
...
export default function ReceivingReportPrintout({
  match,
  reprint = false,
  duplicate = false,
}) {
    ...
    >
      <div>
        ...
        <Row className="m-t-1">
          ...
          <Col span={8} className="is-flex align-items-flex-end">
            <span className="ref-number">{state.rr_no}</span>
          </Col>
        </Row>
        <Row>
          {state.customer || state.supplier
            ? [
                <Col key="label" span={4}>
                  {state.supplier ? "Supplier" : "Customer"}:
                </Col>,
          ...
          {state.sales_return?.sales_return_no && [
            <Col key="label" span={4}>
              SR #:
            </Col>,
            <Col key="value" span={8}>
          ...
          {state.stock_transfer?.stock_transfer_no && [
            <Col key="label" span={4}>
              ST #:
            </Col>,
            <Col key="value" span={8}>
        ...
        <Row>
          ...
          <Col key="value" span={8}>
            {state.purchase_order?.po_no}
          </Col>
          <Col key="label" span={4}>
            Discount(s):
          </Col>
          <Col key="value" span={8}>
            {(state.discounts || []).map((o) => `${o}%`).join(" / ")}
          </Col>
        </Row>
      ...
      <div className="m-t-1">
        ...
        />
        <Row>
          <Col span={24} className="has-text-right summary">
            Gross Amount : {numberFormat(state.gross_amount)}
          </Col>
        </Row>
        <Row>
          <Col span={24} className="has-text-right summary">
            Discount Amount : {numberFormat(state.total_discount_amount)}
          </Col>
        </Row>
        <Row>
          <Col span={24} className="has-text-right summary">
            Net Amount : {numberFormat(state.total_amount)}
          </Col>
        </Row>
      </div>
