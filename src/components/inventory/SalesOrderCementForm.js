...
import {
  cement_delivery_type_options,
  delivery_type_options,
} from "../../utils/Options";
import {
  computeTotalAmount,
  computeTotalAmountCement,
} from "../../utils/computations";
import ItemsCementField from "../../commons/ItemsCementField";
import filterId from "../../commons/filterId";
const { Content } = Layout;
const { Panel } = Collapse;
const url = "/api/sales-orders-cement/";
const title = "Sales Order Form - Cement";
...
export default function SalesOrderCementForm({ navigate }) {
                  ...
                  onChange={(e) => {
                    ...
                    setState((prevState) => {
                      ...
                      const amount = round(
                        quantity *
                          item.price *
                          (record.unit_of_measure?.packaging || 0)
                      );
        ...
        (isEmpty(state.status?.approval_status) ||
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              const amount = computeTotalAmountCement({
                quantity,
                price,
        ...
        {isEmpty(records) ? (
          ...
          >
            ...
            <SimpleSelectFieldGroup
              ...
              formItemLayout={formItemLayout}
              options={cement_delivery_type_options}
            />

            {state.delivery_type === DELIVERY_TYPE_PICKUP_BODEGA && (
              <SelectFieldGroup
                label="Warehouse"
            ...
            {[DELIVERY_TYPE_COMPANY_DELIVERED].includes(
              state.delivery_type
            ) && (
              <Row>
                <Col span={12}>
                  <TextFieldGroup
                    ...
                    onPressEnter={(e) => {
                      ...
                      axios
                        .post(
                          `/api/purchase-orders-cement/${state.po_no}/po-no`,
                          {
            ...
            {[DELIVERY_TYPE_PICKUP_VESSEL].includes(state.delivery_type) && (
              <Row>
                <Col span={12}>
                  <SelectFieldGroup
                    label="Doc No./Vessel"
                    value={
                      state.vessel_arrival &&
                      `DOC #${state.vessel_arrival?.vessel_arrival_no} - ${state.vessel_arrival?.vessel?.name}`
                    }
                    onSearch={(value) =>
                      onVesselArrivalSearch({
                        value,
                    ...
                    onChange={(index) => {
                      const vessel_arrival =
                        options.vessel_arrivals?.[index] || null;

                      setState((prevState) => ({
                        ...prevState,
                        vessel_arrival,
                        ...(vessel_arrival?.supplier?._id && {
                          company: vessel_arrival.company,
                          purchase_order: {
                            ...vessel_arrival,
                            supplier: vessel_arrival.supplier,
                          },
                          items: [
                            ...vessel_arrival.items.map((o) => {
                              const po_detail_id = o?.po_detail_id;
                              const _o = filterId(o);
                ...
                <Col span={12}>
                  <TextFieldGroup
                    ...
                    label="Vessel"
                    name="vessel"
                    value={state.vessel_arrival?.vessel?.name || ""}
                    formItemLayout={smallFormItemLayout}
                  />
                </Col>
              </Row>
