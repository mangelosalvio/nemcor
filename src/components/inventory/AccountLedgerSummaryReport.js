...
const title = "Account Ledger Summary Report";
...
export default function AccountLedgerSummaryReport() {
  ...
  const records_column = [
    ...
    },
    {
      ...
      align: "right",
      render: (o, record) => (
        <span>{record.amount > 0 && `${numberFormat(o)}`}</span>
      ),
    },
    {
      title: "Credit",
      ...
      align: "right",
      render: (o, record) => (
        <span>{record.amount <= 0 && `${numberFormat(Math.abs(o))}`}</span>
      ),
    },
    {
      title: "Balance",
  ...
  const getReport = useCallback(({ ...form_data }) => {
    if (
      ...
      form_data.account?._id
    ) {
      const loading = message.loading("Loading...");
      axios
        .post(`${url}account-ledger-summary`, form_data)
        .then((response) => {
          loading();
          if (response.data) {
