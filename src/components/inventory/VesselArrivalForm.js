...
const url = "/api/vessel-arrivals/";
const title = "Vessel Arrival";
...
const transaction_counter = {
  label: "Doc No.",
  key: "vessel_arrival_no",
};

const date_fields = ["date", "due_date"];

export default function VesselArrivalForm({ navigate }) {
                  ...
                  ...(purchase_order?.supplier?._id && {
                    ...
                    items: [
                      ...purchase_order.items.map((o) => {
                        ...
                        delete _o.confirmed_quantity;
                        delete _o.balance;

                        const freight = round(
                          balance *
                            o.freight_per_unit *
                            (o.unit_of_measure?.packaging || 0)
                        );
            ...
            <Row>
              <Col span={12}>
                <SelectFieldGroup
                  ...
                  onSearch={(value) => onVesselSearch({ value, setOptions })}
                  onChange={(index) => {
                    const vessel = options.vessels?.[index] || null;
                    setState((prevState) => ({
                      ...prevState,
                      vessel,
                    }));
