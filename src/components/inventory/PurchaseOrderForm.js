...
import { computeFreightAndAmount } from "../../utils/computations";
const { Content } = Layout;
const { Panel } = Collapse;
const url = "/api/purchase-orders/";
const title = "Purchase Order Form";
...
export default function PurchaseOrderForm({ navigate }) {
...

    {
      ...
      render: (value, record) => (
        <Input
          ...
          defaultValue={value}
          onBlur={(e) => {
            const invoice_no = e.target.value;
            axios
              .post(`/api/purchase-orders/${record._id}/invoice-no`, {
                invoice_no,
              })
              .then(() => {
                return message.success("PO Invoice# Updated");
              })
              .catch((err) => {
      ...
      render: (value, record, index) => (
        <span>
          ...
          isEmpty(state.deleted) ? (
              ...
              <Col span={24}>
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    const { freight, amount } = computeFreightAndAmount({
                      quantity,
            ...
            record.footer !== 0 && `${numberFormat(record.quantity)}`
          )}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (value, record, index) => (
        <span>
          {record.footer !== 1 &&
          (isEmpty(state.status) ||
            [OPEN, STATUS_ALLOW_PRICE_EDIT].includes(
          ...
          isEmpty(state.deleted) ? (
            ...
            record.footer !== 1 && numberFormat(record.price, 6)
          )}
        </span>
      ),
    },
        ...
        {isEmpty(records) ? (
          ...
          >
              ...
              <Col span={12}>
                <SelectFieldGroup
                  ...
                  onChange={(index) => {
                    ...
                    axios
                      .post("/api/products/product-categories", {
                        product_categories: (
            ...
            <FormButtons
              ...
              setState={setState}
              setItem={setItem}
              save_statuses={[STATUS_ALLOW_PRICE_EDIT]}
              onDelete={() => {
                onDelete({
                  id: state._id,
                  url,
              ...
              onPrint={() => {
                window.open(`/print/purchase-orders/${state._id}`, "_tab");
              }}
              has_save={can_edit}
              additional_buttons={[
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        onUpdateStatus({
                          url,
                          state,
                          approval_status: STATUS_ALLOW_PRICE_EDIT,
                          user: auth.user,
                          cb: () => {
                            edit({
                              record: state,
