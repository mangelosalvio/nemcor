...
const records_column = [
...

  {
    ...
    render: (value, record) => (
      <span>
        {record.footer !== 1 && `${numberFormat(record.freight_per_unit, 3)}`}
      </span>
    ),
  },
...
export default function SupplierWithdrawalPrintout({
  match,
  reprint = false,
  duplicate = false,
}) {
    ...
    >
      <div>
        ...
        <Row className="m-t-1">
          ...
          <Col span={8} className="is-flex align-items-flex-end">
            <span className="ref-number">{state.ww_no}</span>
          </Col>
        </Row>
        <Row>
          ...
          <Col key="value" span={8} className="report-customer-name">
            {state.supplier?.name} / {state.purchase_order?.area?.name}
          </Col>
          <Col key="label" span={4}>
            Terms:
          </Col>
