...
const title = "PO to Collection Report";

const initialValues = {
  period_covered: null,
};
export default function PurchaseOrderToCollectionReport({
  other_set = false,
  history,
}) {
  ...
  const getReport = useCallback(
    ({ ...form_data }) => {
      setIsLoading(true);
      axios
        .post(`${url}purchase-order-to-collection-report`, form_data)
        .then((response) => {
          setIsLoading(false);
          if (response.data) {
