...
const url = "/api/sales/";
const title = "Cash Count Report";

const initialValues = {
  period_covered: [moment().startOf("month"), moment().endOf("month")],
};

const onReprint = ({ record, other_set }) => {
  axios
    .post("/api/sales/reprint/cash-count", {
      ...record,
      other_set: other_set || false,
    })
    .then(() => {
...
export default function CashCountReport({ other_set = false, history }) {
  ...
  const records_column = [
        ...
        items && (
          <Table
            ...
            dataSource={addKeysToArray([
              ...
              {
                footer: 1,
                amount: sumBy(items, (o) => round(o.amount)),
                quantity: sumBy(items, (o) => round(o.quantity)),
              },
            ])}
    ...
    },
    {
      title: "Zread #",
      width: 80,
      dataIndex: ["zread", "zread_id"],
    },
    {
      title: "Cash Sales",
      dataIndex: ["zread", "cash_sales"],
      align: "right",
    ...
    },
    {
      ...
      align: "right",
      render: (value, record) =>
        value && numberFormat(record.total_amount - value),
    },
    {
      title: "",
      width: 30,
        ...
        <span>
          {record.footer !== 1 && (
            <i
              ...
              onClick={() => onReprint({ record, other_set })}
            ></i>
          )}
        </span>
      ),
  ...
  useEffect(() => {
    ...
    if (state.period_covered) {
      ...
      axios
        .post(`${url}cash-count-report`, form_data)
        .then((response) => {
          loading();
          if (response.data) {
        ...
        <div ref={report}>
          ...
          <Table
            ...
            dataSource={addKeysToArray([
              ...
              {
                footer: 1,
                total_amount: sumBy(records, (o) => round(o.total_amount)),
              },
            ])}
