...
const url = "/api/sales/";
const title = "Consolidated Gross Sales Report";

const initialValues = {
  period_covered: [moment().startOf("month"), moment().endOf("month")],
};
export default function ConsolidatedGrossSalesReport({ history }) {
  ...
  useEffect(() => {
    ...
    if (state.period_covered) {
      ...
      axios
        .post(`${url}consolidated-gross-sales-report`, form_data)
        .then((response) => {
          loading();
          if (response.data) {
