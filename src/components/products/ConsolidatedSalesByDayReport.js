...
const url = "/api/sales/";
const title = "Consolidated Sales By Day Report";

const initialValues = {
  period_covered: [moment().startOf("week"), moment().endOf("week")],
};

export default function ConsolidatedSalesByDayReport({
  other_set = false,
  history,
}) {
  ...
  useEffect(() => {
    ...
    if (state.period_covered) {
      ...
      axios
        .post(`${url}consolidated-sales-by-day-report`, form_data)
        .then((response) => {
          loading();
  ...
  }, [state.period_covered, other_set, history, auth.user]);
          ...
          <Table
            ...
            dataSource={addKeysToArray([
              ...records,
              {
                ...
                net_of_vat: sumBy(records, (o) => round(o.net_sales || 0)),
              },
            ])}
            pagination={false}
