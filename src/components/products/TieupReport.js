...
const url = "/api/sales/";
const report_url = "tieup-report";
const title = "Tie-up Report";
...
const records_column = [
  ...
  {
    title: "Tieup",
    dataIndex: ["tieup_information", "tieup", "name"],
  },
  {
    title: "Booking Reference",
    dataIndex: ["tieup_information", "booking_reference"],
  },
  {
    title: "List of Items",
    dataIndex: ["items"],
...
export default function TieupReport({ other_set = false, history }) {
  ...
  const onTieupChange = (index) => {
    setState((prevState) => {
      return {
        ...prevState,
        tieup: prevState.options.tieups[index],
      };
    });
  };
  ...
  }, [state.period_covered, other_set, auth.user, history, state.tieup]);
          ...
          <Col span={12}>
            <Form>
              <SelectFieldGroup
                ...
                onChange={onTieupChange}
                onSearch={onTieupSearch}
                formItemLayout={smallFormItemLayout}
                data={state.options.tieups}
                autoFocus={true}
              />
            </Form>
          </Col>
