...
import { Layout, Table, Form, Button, message, Breadcrumb } from "antd";
...
class VoidedAccountPaymentListings extends Component {
  state = {
    title: "Products",
    url: "/api/products/",
    search_keyword: "",
  ...
  render() {
    ...
    const records_column = [
      ...
      {
        title: "REASON",
        dataIndex: "deleted.reason",
      },

      {
        title: "DELETED",
        dataIndex: "deleted.user.name",
        render: (name, record) => (
          <div>
            {name} <br />
            {record.deleted && moment(record.deleted.datetime).format("LLL")}
          </div>
        ),
      },
    ];
        ...
        <div className="columns is-marginless">
          <div className="column">
            <Breadcrumb style={{ margin: "16px 0" }}>
              ...
              <Breadcrumb.Item>Voided Account Payments</Breadcrumb.Item>
            </Breadcrumb>
          </div>
        </div>
        ...
        <div style={{ background: "#fff", padding: 24, minHeight: 280 }}>
          <div>
            <span className="has-text-weight-bold" style={{ fontSize: "18px" }}>
              VOIDED ACCOUNT PAYMENT LISTINGS
            </span>
          </div>
          <hr />
          <div>
          ...
          <div ref={(el) => (this.report = el)}>
            <div className="has-text-centered report-heading">
              Voided Account Payment Listings <br />
...
export default connect(mapToState)(VoidedAccountPaymentListings);
