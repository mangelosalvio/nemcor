...
class Tables extends Component {
  ...
  render() {
    ...
    if ([USER_OWNER].includes(this.props.auth.user?.role)) {
      records_column.splice(1, 0, {
        title: "Is Other Set",
        dataIndex: "is_other_set",
        align: "center",
        render: (value) => (
          <span>
            {value && (
        ...
        <div style={{ background: "#fff", padding: 24, minHeight: 280 }}>
          ...
          {isEmpty(this.state[collection_name]) ? (
            <Form onSubmit={this.onSubmit}>
              ...
              {[USER_OWNER].includes(this.props.auth.user?.role) && (
                <CheckboxFieldGroup
                  label="Is Other Set"
                  name="is_other_set"
                  onChange={this.onChange}
...
export default connect(mapToState)(Tables);
